...
// Configure Tinify
console.log(
  'Tinify API Key configured:',
  process.env.TINIFY_API_KEY ? 'Yes' : 'No'
)

class ImageUploadService {
  constructor() {
    this.imgbbApiUrl =
      process.env.IMG_BB_API_URL || 'https://api.imgbb.com/1/upload'
    ...
    try {
      ...
      if (process.env.TINIFY_API_KEY) {
        try {
          ...
          console.warn(
            'Tinify optimization failed, using original image:',
            tinifyError.message
          )
  ...
  validateImage(imageBuffer, filename) {
    ...
    if (!allowedExtensions.includes(ext)) {
      throw new Error(
        `Unsupported image format. Allowed: ${allowedExtensions.join(', ')}`
      )
    }
