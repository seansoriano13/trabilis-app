...
export function hasBookingDeparted(booking) {
    if (
        !booking.amadeus_flight_offer?.itineraries?.[0]?.segments?.[0]
            ?.departure?.at
    ) {
        return false
    }
...
export async function cancelFlightBooking(
    ...
    try {
        ...
        if (hasBookingDeparted(booking)) {
            throw new Error(
                'Cannot cancel booking: Flight has already departed'
            )
        }
...
export async function getCancellationInfo(bookingReference) {
    try {
        ...
        const blockReason = canCancel
            ? null
            : getCancellationBlockReason(booking)

        return {
            canCancel: canCancel && !hasDeparted,
            hasDeparted,
            blockReason: hasDeparted
