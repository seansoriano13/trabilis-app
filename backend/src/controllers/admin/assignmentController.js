  ...
  try {
    ...
    if (!bookingType || !bookingId || !assignedTo || !assignedBy) {
      return res.status(400).json({
        success: false,
        message:
          'bookingType, bookingId, assignedTo, and assignedBy are required',
      })
    }
    ...
    if (!validTypes.includes(bookingType)) {
      return res.status(400).json({
        success: false,
        message:
          'Invalid booking type. Must be one of: tour, flight, visa-inquiry, visa-processing',
      })
    }
...
export const updateAssignmentStatus = async (req, res) => {
  try {
    ...
    if (updateError) {
      console.error(
        `Error updating ${bookingType} assignment status:`,
        updateError
      )
      return res.status(500).json({
        success: false,
  ...
  try {
    ...
    if (!bookingType || !bookingId || !newStaffId || !assignedBy) {
      return res.status(400).json({
        success: false,
        message:
          'bookingType, bookingId, newStaffId, and assignedBy are required',
      })
    }
    ...
    const result = await manualReassignBooking(
      bookingType,
      bookingId,
      newStaffId,
      assignedById
...
const getBookingReference = async (bookingType, bookingId) => {
  ...
  const referenceField =
    bookingType === 'visa-inquiry' ? 'inquiry_reference' : 'booking_reference'
  ...
  switch (bookingType) {
    case 'tour':
      return {
        ...
        packageTitle:
          booking.package_dates?.tour_packages?.title || 'Unknown Package',
      }
    case 'flight':
      return {
        bookingReference: booking.booking_reference,
        ...
        assignedTo: assignedTo,
        destination:
          booking.search_criteria?.destination || 'Unknown Destination',
      }
    case 'visa-inquiry':
      return {
