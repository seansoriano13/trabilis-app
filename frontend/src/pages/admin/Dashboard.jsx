...
const Dashboard = () => {
  ...
  const currentMonthLabelLong = new Date().toLocaleString('en-US', {
    month: 'long',
  })
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Only set loading states if this is not the initial load
      ...
      try {
        const flightPromise = supabase
          .from('flight_bookings')
          .select('*', { count: 'exact' })
          .range(flightPage * pageSize, (flightPage + 1) * pageSize - 1)
        ...
        const aggregateMonth = async (date) => {
          ...
          const flightSum = (flightAgg.data || []).reduce(
            (s, r) => s + (Number(r.total_amount) || 0),
            0
          )
          const tourSum = (tourAgg.data || []).reduce(
            (s, r) => s + (Number(r.total_amount) || 0),
            0
          )
        ...
        if (!normalizedRevenue.length) {
          ...
          const now = new Date()
          const thisShort = now.toLocaleString('en-US', {
            month: 'short',
          })
          // Try to match by short month label, numeric month, or full string includes
          const mm = (now.getMonth() + 1).toString().padStart(2, '0')
          const found = normalizedRevenue.find(
            (r) =>
              (r.month || '').toString().toLowerCase() ===
                thisShort.toLowerCase() ||
        ...
        try {
          const todayBookingsCount =
            (todayFlightRes.count || 0) + (todayTourRes.count || 0)
      ...
      <div className='dashboard__section'>
        <div className='dashboard__section-header'>
          <div className='dashboard__section-title'>
            ...
            {searchLoading && (
              <div className='dashboard__search-loading'>
                <div className='dashboard__search-spinner'></div>
                <span>Loading...</span>
              </div>
            )}
          </div>
          ...
          <table className='dashboard__table'>
            <thead>
              <tr>
                ...
                <th
                  onClick={() =>
                    handleSortFlight('search_criteria.destination')
                  }
                  className='dashboard__table-header dashboard__table-header--sortable'
                >
                  <div className='dashboard__table-header-content'>
                ...
                <th
                  onClick={() =>
                    handleSortFlight('search_criteria.outboundDeparture')
                  }
                  className='dashboard__table-header dashboard__table-header--sortable'
                >
                  <div className='dashboard__table-header-content'>
