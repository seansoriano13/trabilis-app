    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (sort.key === 'updated_at') {
          ...
          query = query.order('search_criteria->>outboundDeparture', {
            ascending: true,
          })
        }
        ...
        if (filters.lead_name) {
          query = query.or(
            `passenger_details->>firstName.ilike.%${filters.lead_name}%,passenger_details->>lastName.ilike.%${filters.lead_name}%`
          )
        }
        ...
        if (bookingsRes.error) {
          ...
          throw new Error(
            `Failed to fetch flight bookings: ${bookingsRes.error.message}`
          )
        }
        ...
        if (statsRes.error) {
          ...
          throw new Error(
            `Failed to fetch flight stats: ${statsRes.error.message}`
          )
        }
  ...
  const handleEditSubmit = async (submitData) => {
    try {
      const response = await adminClient.put(
        `/flights/${selectedBooking.id}/edit`,
        submitData
      )
          ...
          <table className='flights__table'>
            <thead>
              <tr>
                ...
                <th
                  onClick={() =>
                    handleSort('search_criteria.outboundDeparture')
                  }
                  className='flights__table-header flights__table-header--sortable'
                >
                  <div className='flights__table-header-content'>
                ...
                >
                  ...
                  <td className='flights__table-cell flights__table-cell--flight'>
                    <div className='flights__flight-details'>
                      <div className='flights__departure'>
                        <FiClock size={14} />
                        <span>
                          {booking.amadeus_flight_offer?.itineraries[0]
                  ...
                  <td className='flights__table-cell flights__table-cell--passenger'>
                    <div className='flights__passenger-info'>
                      ...
                      <span>
                        {
                          booking.passenger_details?.travelers[0]?.name
                            ?.firstName
                        }{' '}
                        {
                          booking.passenger_details?.travelers[0]?.name
                    ...
                    <div className='flights__assignment'>
                      {booking.assigned_to ? (
                        <div className='flights__assignment-assigned'>
                          ...
                          <span
                            className={`flights__assignment-status flights__assignment-status--${booking.assignment_status}`}
                          >
                        ...
                        <span className='flights__assignment-unassigned'>
                          Unassigned
                        </span>
                      )}
                    </div>
